        ...
        case 2:
            switch(x[x.length - 2]) {
                case "add":
                    ...
                    switch(args[0]) {
                        case 'badDiscord':
                            s = `Argument is neither a Discord ID nor a mention.` + s;
                            break;

                        case 'notFoundDiscord':
                            s = `Discord user not found.` + s;
                            break;
...
async function add(msg, args, client) {
    ...
    // Discord user not found in guild
    if(discordUser == undefined) {
        args[0] = 'notFoundDiscord';
        badArgument(2, msg, args, client, `add ${albionUsername}`);
        return;
    }
...
async function get(msg, args, client) {
    ...
    x = undefined;
    try {
        x = player.replace("!", "").replace("<@", "").replace(">", "");    // Nicked users are '<@!...>', while unnicked users are '<@...>'.
    } catch {}

    s = '', found = false, discordUser = undefined;
    for(const [albionUsername, discordID] of Object.entries(config.aliases)) {
    ...
    if(found) {
        userTag = 'Found: ';
        try {   // While testing, Discord user might not be in server.
            userTag = (await msg.guild.members.fetch(discordUser)).user.tag;
            userTag = `Found: ${userTag}`;
        } catch {}
        await msg.channel.send(`${userTag}\n${s}`);
        return;
    }
    badArgument(1, msg, args, client, "get");
}
